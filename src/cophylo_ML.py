...
def run_model(df):
  ...
  return(history, predicted_values, labels_test_set)
...
### model calls below here
outfile_base = '/Users/waded/Documents/cophyloML/vignettes/cophyloML_ext'
ext_rates = ["0", "0.1", "0.2"]
csp_rates = ["0", "1", "2"]
hs_rates = ["0", "1", "2"]
histories = []
...
for e in ext_rates:
  for c in csp_rates:
    for hs in hs_rates:
      out_file = outfile_base + e + "_csp" + c + "_hs" + hs + ".csv" 
      df = pd.read_csv(out_file)
      history, predicted_vals, test_labs = run_model(df)
